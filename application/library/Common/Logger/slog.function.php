...
function slog($log,$type='log',$css='')
{
    if(is_string($type))
    {
        ...
        if(method_exists('\Common\Logger\Slog',$type) || in_array($type,Slog::$log_types))
        {
           return  call_user_func(array('\Common\Logger\Slog',$type),$log,$css);
        }
    }
