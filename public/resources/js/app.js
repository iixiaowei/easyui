	function nbox(title,type){
		if(type==0){
			$.smallBox({
			      title: "<i class='fa fa-times'></i> "+title,
	...
	function logout(){
		...
		var $this = $('#admLogout');
		$.loginURL = "index.php?m=admin&c=login&a=logout";
		$.logoutMSG = $this.data('logout-msg');
		
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> 退出登录 <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
			content : $.logoutMSG || "关闭打开的浏览器，可以提高您的系统安全性。",
			buttons : '[取消][确定]'
...
$(document).ready(function() {
	...
	$('.minifyme').click(function(e) {
		...
		$("#contentSwap").contents().find("body").toggleClass("minified");
	...
	$('#refresh').click(function(e) {
		$.SmartMessageBox({
			...
			content : "你要重置所有你保存的小工具和本地缓存中的信息?",
			buttons : '[取消][确定]'
		}, function(ButtonPressed) {
